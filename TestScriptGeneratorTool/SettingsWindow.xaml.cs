...
namespace TestScriptGeneratorTool
{
    ...
    public partial class SettingsWindow : Window
    {
        ...
        private async Task LoadApiKeyForProvider(string provider)
        {
            try
            {
                ...
                if (await _credentialService.CredentialExistsAsync(credentialKey))
                {
                    ...
                    if (!string.IsNullOrEmpty(apiKey))
                    {
                        ...
                        _logger.LogInformation($"Loaded API key for provider: {provider}");
                    }
                }
            }
            catch (Exception ex)
            {
                _logger.LogError($"Failed to load API key for provider {provider}: {ex.Message}");
            }
        }
        ...
        private void ProviderComboBox_SelectionChanged(object sender, SelectionChangedEventArgs e)
        {
            try
            {
                ...
                {
                    _ = LoadApiKeyForProvider(selectedProvider);
                }
            }
            catch (Exception ex)
            {
        ...
        private async void SaveButton_Click(object sender, RoutedEventArgs e)
        {
            try
            {
                ...
                // IMPORTANT: Trim API key to remove any accidental spaces or newlines
                ...
                if (!string.IsNullOrEmpty(apiKey))
                {
                    ...
                    _logger.LogInformation($"API key stored and set for provider: {_aiSettings.Provider} (length: {apiKey.Length})");
                }
                else
                {
                    _logger.LogWarning("API key is empty after trimming");
                }
